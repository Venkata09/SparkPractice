...
object Window_Test {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val sparkSession: SparkSession = SparkSession.builder().master("local[4]").appName("<<<<<< Windows - Test  >>>>>>").getOrCreate()
    ...
    val wspec = Window.partitionBy("name").orderBy($"timestamp".asc)
    val interpolatedUsage_1 = interpolatedUsage.withColumn("timestampPrev", lag($"timestamp", 1).over(wspec))
      .withColumn("GBsecs", ($"timestamp" - $"timestampPrev") * $"gb")
      .withColumn("VCoresecs", ($"timestamp" - $"timestampPrev") * $"cpus")
